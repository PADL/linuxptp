...
static int subdomain_to_handle(const Octet subdomain[PTP_V1_SUBDOMAIN_NAME_LENGTH])
{
	if (memcmp(subdomain, PTP_V1_SUBDOMAIN_DFLT, PTP_V1_SUBDOMAIN_NAME_LENGTH) == 0)
		...
		return (int)(subdomain_crc(subdomain) % 3U);
}
...
static int TimeRepresentation_to_Timestamp(const struct TimeRepresentation *tr,
					   ...
					   struct Timestamp *ts)
{
	if ((int32_t)ntohl(tr->nanoseconds) < 0)
		return -EBADMSG;
			   ...
			   bool sync_as_announce)
{
	...
	if ((v2_hdr->tsmt & 0x0F) == SYNC && sync_as_announce) {
		v2_hdr->tsmt &= ~(0x0F);
	...
	if (v1_msg->hdr.control == PTP_V1_MESSAGE_SYNC ||
	    v1_msg->hdr.control == PTP_V1_MESSAGE_DELAY_REQ) {
		v1_msg_to_flagField(&v1_msg->sync, v2_hdr->flagField);
	} else if (v1_msg->hdr.control == PTP_V1_MESSAGE_FOLLOW_UP) {
	...
	case SYNC:
		/* fallthrough */
	case ANNOUNCE:
		/* one synthetic ANNOUNCE is derived from each PTPv1 SYNC */
		v2_hdr->logMessageInterval = v1_msg->sync.syncInterval;
		break;
	default:
		/*
		 * PTPv1 leaves the delay request interval to the client, so
		 * there is nothing to convey here. Mark it unspecified, as
		 * PTPv2 does, and let the port keep the interval it was
		 * configured with.
		 */
		v2_hdr->logMessageInterval = 0x7f;
		break;
	}
				...
				size_t max_count)
{
	...
	memcpy(&path[*count], &ci, sizeof(ci));
	(*count)++;
				  ...
				  struct announce_msg *v2_announce)
{
	...
	count = 0;
	max_path_entries = (sizeof(v2_announce->suffix) - sizeof(struct TLV)) /
		sizeof(struct ClockIdentity);

	err = merge_path_trace_tlv(v1_sync->grandmasterCommunicationTechnology,
				   v1_sync->grandmasterClockUuid,
				   path, &count, max_path_entries);
	if (err == 0)
...
static int get_path_trace_tlv(const struct announce_msg *v2_announce,
			      struct ClockIdentity **path)
{
	size_t length = ntohs(v2_announce->hdr.messageLength);
	const uint8_t *suffix = v2_announce->suffix;

	if (length < sizeof(*v2_announce))
		return -EBADMSG;

	length -= sizeof(*v2_announce);

	while (length >= sizeof(struct TLV)) {
		const struct TLV *tlv = (const struct TLV *)suffix;
		size_t tlv_length = ntohs(tlv->length);

		if (tlv_length > length - sizeof(struct TLV))
			return -EBADMSG;

		if (ntohs(tlv->type) == TLV_PATH_TRACE) {
			if ((tlv_length % sizeof(struct ClockIdentity)) != 0)
				return -EBADMSG;

			*path = (struct ClockIdentity *)tlv->value;
			return tlv_length / sizeof(struct ClockIdentity);
		}

		suffix += sizeof(struct TLV) + tlv_length;
		length -= sizeof(struct TLV) + tlv_length;
	}
			 ...
			 struct sync_delay_req_msg_v1 *v1_sync)
{
	...
	/*
	 * Use the announce source when we are the grandmaster, or when
	 * there is no path trace to derive the parent from.
	 */
	if (v1_sync->hdr.control == PTP_V1_MESSAGE_DELAY_REQ ||
	    path_count <= 1) {
