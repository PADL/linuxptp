			   ...
			   struct ptp_header *v2_hdr)
{
	...
	case PTP_V1_MESSAGE_MANAGEMENT: {
		...
		param_length = ntohs(v1_msg->management.parameterLength);
		if (param_length > PTP_V1_MAX_MANAGEMENT_PAYLOAD_SIZE ||
		    expected_length > SIZE_MAX - param_length)
			return -EBADMSG;
...
static int get_path_trace_tlv(const struct announce_msg *v2_announce,
			      struct ClockIdentity **path)
{
	ssize_t length = ntohs(v2_announce->hdr.messageLength) - sizeof(*v2_announce);
	...
	while (length >= sizeof(struct TLV)) {
		...
		if (ntohs(tlv->type) != TLV_PATH_TRACE) {
			suffix += sizeof(tlv->type) + sizeof(tlv->length) + tlv_length;
			length -= sizeof(tlv->type) + sizeof(tlv->length) + tlv_length;
			if (length < 0)
				return -EBADMSG;
				     ...
				     struct management_tlv *v2_mgmt)
{
	...
	if (ntohs(v1_mgmt->parameterLength) < sizeof(*v1_dds))
		return -EBADMSG;
				     ...
				     struct management_tlv *v2_mgmt)
{
	...
	if (ntohs(v1_mgmt->parameterLength) < sizeof(*v1_cds))
		return -EBADMSG;
				    ...
				    struct management_tlv *v2_mgmt)
{
	...
	if (ntohs(v1_mgmt->parameterLength) < sizeof(*v1_pds))
		return -EBADMSG;
			       ...
			       struct management_msg_v1 *v1_mgmt)
{
	...
	if (entry->v2_to_v1) {
		...
		if (err == 0 &&
		    ntohs(v1_mgmt->parameterLength) > PTP_V1_MAX_MANAGEMENT_PAYLOAD_SIZE)
			return -ERANGE;
