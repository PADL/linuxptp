...
static struct in6_addr ipv6_mcast_addr[MC_ADDR_COUNT] = {
	...
	{ { "\xFF\x0E\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x82" } },
	{ { "\xFF\x0E\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x83" } },
	{ { "\xFF\x0E\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x84" } },
};

/*
 * Returns TRUE if ptp4l instantiated the transport. Only ptp4l supplies
 * an application context, and only ptp4l drains the synthetic ANNOUNCE
 * described in v1_transport_recv().
 */
...
static int v1_get_mcast_address(const struct v1_transport *v1, struct address *addr)
{
	...
	handle++;
	if (handle < 0 || handle >= MC_ADDR_COUNT)
		return -ERANGE;
...
static int v1_open_socket(struct interface *iface, int event,
			  sa_family_t family, const struct address *mc_addr,
			  short port, int *interface_index,
			  int ttl_or_hop_limit, enum timestamp_type ts_type)
{
	int fd;
	...
	*interface_index = -1;
	...
	switch (family) {
	case AF_INET:
		fd = udp_open_socket(iface, event, &mc_addr->sin.sin_addr,
				     1, port, ttl_or_hop_limit, ts_type,
				     TRANS_V1_UDP_IPV4_NP);
		break;
	case AF_INET6:
		fd = udp6_open_socket(iface, event, &mc_addr->sin6.sin6_addr,
				      1, port, interface_index,
				      ttl_or_hop_limit, ts_type,
				      TRANS_V1_UDP_IPV6_NP);
		break;
	default:
		errno = EAFNOSUPPORT;
		return -1;
			     ...
			     sa_family_t family, const char *mcast_option)
{
	 ...
	 * multicast addresses. This may be overridden by the configuration.
	 */
	...
	} else if (v1_get_mcast_address(v1, &v1->mcast)) {
		pr_err("no PTPv1 subdomain mapping for domainNumber %u",
		       v1->context.domain_number);
		goto no_event;
	}
	...
	efd = v1_open_socket(iface, 1, family, &v1->mcast, EVENT_PORT,
			     &v1->index, ttl, ts_type);
	if (efd < 0)
		goto no_event;

	gfd = v1_open_socket(iface, 0, family, &v1->mcast, GENERAL_PORT,
			     &v1->index, ttl, ts_type);
	if (gfd < 0)
		goto no_general;
			     ...
			     struct address *addr, struct hw_timestamp *hwts)
{
	...
	if (buflen < (int)sizeof(struct message_data))
		return -ERANGE;
	...
	if (fd == -1) {
		 ...
		 */
		if (buflen > (int)sizeof(v1->context.last_announce_rx.data.buffer))
			return -EMSGSIZE;
	...
	ret = v1_message_to_v2(&v1->context, &v1_msg_buf, buf, false);
	if (ret < 0) {
		if (ret != -EPROTO)
	...
	if (msg_type(buf) == SYNC) {
		...
		ret = v1_message_to_v2(&v1->context, &v1_msg_buf,
				       &v1->context.last_announce_rx, true);
		if (ret >= 0)
			t->pending_msg = 1;
	}
			     ...
			     struct address *addr, struct hw_timestamp *hwts)
{
	...
	case AF_INET6:
		...
		if (is_link_local(&mcast.sin6.sin6_addr))
			mcast.sin6.sin6_scope_id = v1->index;
		break;
	}
	 ...
	 */
	return (cnt == (ssize_t)v1_msg.length) ? len : 1;
}
