...
static int v1_open_socket(const char *name, sa_family_t family,
			  const struct address *mc_addr,
			  ...
			  int ttl_or_hop_limit)
{
	...
	*interface_index = -1;
	...
	switch (family) {
	case AF_INET:
		fd = udp_open_socket(name, &mc_addr->sin.sin_addr,
				     1, port, ttl_or_hop_limit);
		break;
	case AF_INET6:
		fd = udp6_open_socket(name, &mc_addr->sin6.sin6_addr,
			     ...
			     sa_family_t family, const char *mcast_option)
{
	...
	efd = v1_open_socket(name, family, &v1->mcast, EVENT_PORT,
			     &v1->index, ttl);
	if (efd < 0)
		goto no_event;

	gfd = v1_open_socket(name, family, &v1->mcast, GENERAL_PORT,
			     &v1->index, ttl);
	if (gfd < 0)
		goto no_general;
