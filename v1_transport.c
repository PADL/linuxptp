/**
 * @file v1_transport.c
 * @brief Implements a IEEE1588-2002 PTP message translation layer.
...
{
	return v1->t.context != NULL && port_event != NULL;
}
			     ...
			     sa_family_t family, const char *mcast_option)
{
	 ...
	 */
	port->versionNumber = PTP_V1_VERSION_PTP;

	v1->context.flags = 0;
	v1->context.domain_number = config_get_int(t->cfg, NULL, "domainNumber");
	v1->context.ts = port->transportSpecific;
	v1->context.port_identity = &port->portIdentity;
	v1->context.clock = port->clock;
